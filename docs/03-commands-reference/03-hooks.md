...
The `BeforeCommandRegister` hook runs on the **client** when command definitions are being processed and registered into the client registry. It allows you to filter which commands are visible and available in the UI and autocomplete menu for a specific user.
...
:::warning Client-Only Hook

`BeforeCommandRegister` runs strictly on the client. Because exploiters can bypass client-side code, hiding a command via `BeforeCommandRegister` does **not** secure server commands. Always enforce backend authorization checks using a `BeforeRun` hook on the server.
